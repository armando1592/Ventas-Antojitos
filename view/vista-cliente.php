...
function agregarAlCarrito(id, nombre, precio, stock) {
    ...
    const productoExistente = carritoVentas.find(item => item.id === id);
    
    if (productoExistente) {
        // Si existe, incrementar cantidad (verificando stock)
...
function actualizarCarrito() {
    const listaCompra = document.getElementById('lista_compra');
    ...
    // Generar filas del carrito
    let html = '';
    carritoVentas.forEach(item => {
        const subtotalItem = item.precio * item.cantidad;
        html += `
            <tr>
                <td style="font-weight: 600; font-size: 0.85rem;">${item.nombre}</td>
                <td>
                    <input type="number" 
                ...
                <td style="font-weight: 600; color: #667eea;">S/ ${item.precio.toFixed(2)}</td>
                <td style="font-weight: 700; color: #11998e;">S/ ${subtotalItem.toFixed(2)}</td>
                <td>
...
function actualizarCantidad(id, nuevaCantidad, stock) {
    ...
    const producto = carritoVentas.find(item => item.id === id);
    if (producto) {
...
function realizarVenta() {
    ...
    console.log('Procesando venta:', carritoVentas);
    
    // Ejemplo de confirmación
    if (confirm('¿Confirmar la venta?')) {
        // Aquí iría tu lógica de guardar la venta en la base de datos
